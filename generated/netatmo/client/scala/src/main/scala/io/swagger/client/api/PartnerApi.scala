...
class PartnerApi(val defBasePath: String = "https://api.netatmo.net/",
   ...
   */
  def partnerdevices (accessToken: String) : Option[NAPartnerDevicesResponse] = {
    // create path and map variables
    val path = "/api/partnerdevices".replaceAll("\\{format\\}","json")
    ...
    try {
      apiInvoker.invokeApi(basePath, path, "GET", queryParams.toMap, formParams.toMap, postBody, headerParams.toMap, contentType) match {
        case s: String =>
           Some(ApiInvoker.deserialize(s, "", classOf[NAPartnerDevicesResponse]).asInstanceOf[NAPartnerDevicesResponse])
