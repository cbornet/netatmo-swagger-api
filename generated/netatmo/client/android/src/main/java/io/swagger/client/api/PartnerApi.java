...
public class PartnerApi {
   ...
   */
  public NAPartnerDevicesResponse  partnerdevices (String accessToken) throws ApiException {
    Object postBody = null;
    
    // verify the required parameter 'accessToken' is set
    if (accessToken == null) {
       throw new ApiException(400, "Missing the required parameter 'accessToken' when calling partnerdevices");
    }
    

    // create path and map variables
    String path = "/api/partnerdevices".replaceAll("\\{format\\}","json");
    ...
    try {
      String response = apiInvoker.invokeAPI(basePath, path, "GET", queryParams, postBody, headerParams, formParams, contentType);
      if(response != null){
        return (NAPartnerDevicesResponse) ApiInvoker.deserialize(response, "", NAPartnerDevicesResponse.class);
      }
      else {
        return null;
      }
