...
#
package WWW::SwaggerClient::PartnerApi;
...
sub partnerdevices {
    ...
    # verify the required parameter 'access_token' is set
    unless (exists $args{'access_token'}) {
      croak("Missing the required parameter 'access_token' when calling partnerdevices");
    }
    

    # parse inputs
    my $_resource_path = '/api/partnerdevices';
    $_resource_path =~ s/{format}/json/; # default format to json
    ...
    my $_response_object = $self->{api_client}->deserialize('NAPartnerDevicesResponse', $response);
    return $_response_object;
